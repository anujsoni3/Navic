const Metro_model = require("../model/Metro_model");
...
exports.searchMetro = async (req, res) => {
  try {
    ...
    // console.log("📤 Received user input:", { userStart, userEnd, requestedTime });
    ...
    const metroLines = await Metro_model.find({});
    ...
    for (const line of metroLines) {
      ...
      if (startIndex !== -1 && endIndex !== -1 && startIndex < endIndex) {
        ...
        if (possibleTimes.length > 0) {
          ...
          if (arrivalTime) {
            ...
            console.log("✅ Found exact metro:", result);
      ...
      // If fallbackResult not set yet, try to pick the metro line and stations closest to user input ignoring strict time/direction
    ...
    if (fallbackResult) {
      console.log("⚠️ Returning fallback metro:", fallbackResult);
      return res.status(200).json(fallbackResult);
    }

    // No metro found at all
    return res
      .status(404)
      .json({ message: "No metro available at or after given time." });
  } catch (err) {
    console.error("❗ Error in searchMetro:", err);
